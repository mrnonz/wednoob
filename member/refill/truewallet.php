...
//            include ('class.truewallet.php');
            ...
            else
            {
                ...
                $today_use_check_s = $today_year."-".$today_month."-".$today_day;
                $today_day_e = date("d") + 2;
                $today_use_check_e = $today_year."-".$today_month."-".$today_day_e;
                //* END DAY Setup
...
//                    $gettoken = json_decode($wallet->GetToken(),true)['data']['accessToken'];
                    $gettoken = json_decode($wallet->GetToken())->data->accessToken; 
...
//                    if($getTran = json_decode($wallet->getTran($token,'2018-12-01','2018-12-30')))
                    {
                    ...
                    $fti_u = "0";
//                    echo $today_use_check_s." || ".$today_use_check_e."<br/>";
//                    echo count($Tran)." === <br/>";
                    /*echo "<pre>";
                    print_r($Tran);
					echo "</pre>";
                    ...
                    foreach($Tran as $transaction)
                    {
						...
						/*$last_report = $transaction->reportID;
$full_last_report = json_decode($wallet->CheckTran($token,$last_report));
echo "<pre>";
print_r($full_last_report);
echo "</pre>";*/
                    }
//exit();
