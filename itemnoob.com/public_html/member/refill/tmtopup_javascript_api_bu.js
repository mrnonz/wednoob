...
{
					while(pin_code.indexOf('0')!=-1) { pin_code = pin_code.replace('0','H'); }
				while(pin_code.indexOf('1')!=-1) { pin_code = pin_code.replace('1','D'); }
				while(pin_code.indexOf('2')!=-1) { pin_code = pin_code.replace('2','B'); }
				while(pin_code.indexOf('3')!=-1) { pin_code = pin_code.replace('3','F'); }
				while(pin_code.indexOf('4')!=-1) { pin_code = pin_code.replace('4','A'); }
				while(pin_code.indexOf('5')!=-1) { pin_code = pin_code.replace('5','G'); }
				...
				while(pin_code.indexOf('9')!=-1) { pin_code = pin_code.replace('9','J'); }
	return pin_code;
}
...
submit_tmnc = function()
{
	...
	tmtopupForm.action = "https://www.tmtopup.com/topup/?uid=217008";
	...
	input_pid.name = "pid";
	input_pid.value = "1545200680-122491673";
...
function tmtopup_new()
{
	...
	submit_payment = function()
	{
		...
		jQuery.ajax({
			url: protocol + "://www.tmtopup.com/topup/index.php?uid=217008&x=" + Math.random(),
			crossDomain: true,
			cache: false,
			type: 'post',
			data: {
				"pid" : "1545200680-122491673",
				"method" : "3rdTopup",
				...
				"success_url" : "aHR0cDovL3d3dy50bXRvcHVwLmNvbS90b3B1cC9zdWNjZXNzLmh0bWw="
			},
